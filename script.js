...
import { getFirestore, doc, setDoc, getDoc, onSnapshot, collection, getDocs, deleteDoc, serverTimestamp, addDoc, runTransaction }
...
window.togglePublish = async () => {
  ...
  try {
    const snap = await getDoc(doc(db, 'config', 'publishedTorneos'));
    let ids = snap.data()?.sessionIds || [];
    if(isPublished){
      ids = ids.filter(id => id !== sessionId);
      isPublished = false;
      toast('🔒 Torneo ocultado al público');
    } else {
      if(!ids.includes(sessionId)) ids.push(sessionId);
      isPublished = true;
      toast('📡 Torneo publicado — ya visible para todos');
    }
    await setDoc(doc(db, 'config', 'publishedTorneos'), { sessionIds: ids, updatedAt: serverTimestamp() });
    updatePublishBtn();
  } catch(e){
...
function buildPublicCard(sid, tData){
  ...
  card.innerHTML =
    `<div class="pub-card-header">
      <div class="pub-card-title">${gd.title}</div>
...
function renderMergedQueue(queueMap, body){
  ...
  allItems.forEach(it => {
    ...
    div.innerHTML =
      ...
      `<div>`+
        `<div class="qv-teams">${it.t1}<span class="vs">vs</span>${it.t2}</div>`+
        `<div class="qv-group">${it.group}${it.torneoName ? ` · <span style="opacity:0.6">${it.torneoName}</span>` : ''}</div>`+
      `</div>`+
      (it.devName?`<div class="qv-device"><div class="qv-device-name ${it.status}">${it.devName}</div><div class="qv-device-lbl">${isLive?'dispositivo':'siguiente en'}</div></div>`:'');
...
function renderPublicStandings(data, body, sideBy=false){
  ...
  function buildCard(g, width){
    ...
    const rows=st.map((s,rank)=>{
      const gd=s.GF-s.GA;
      return `<tr${rank===0?' class="dv-row-first"':''}><td><span class="dv-rank${rank===0?' top':''}">${rank+1}</span></td><td class="td-name">${s.team.name}</td><td>${s.P>0?s.P:'—'}</td><td class="${gd>0?'dv-dg-pos':gd<0?'dv-dg-neg':''}">${s.P>0?(gd>0?'+'+gd:gd):'—'}</td><td><span class="dv-pts">${s.P>0?s.Pts:'—'}</span></td></tr>`;
    }).join('');
    ...
    card.innerHTML=`<div class="dv-group-name">${g.name}</div><div class="dv-table-wrap"><table class="dv-table"><thead><tr><th style="width:2em">#</th><th class="th-name">Equipo</th><th>PJ</th><th>DG</th><th>Pts</th></tr></thead><tbody>${rows}</tbody></table></div>`;
    return card;
  }
  ...
  function showMatch(data){
    ...
    card.innerHTML = `
      ...
      <div class="ipad-score-area">
        ...
        <div class="ipad-team-row">
          <div class="ipad-team-name">${data.t2}</div>
          <div class="ipad-score-ctrl">
    ...
    if(SESSION_ID){
      ...
      onSnapshot(doc(db,'config','publishedTorneos'), snap=>{
        ...
        ids.forEach(sid=>{
          ...
          onSnapshot(queueRef(sid), qSnap=>{
            pubQueuesLocal[sid] = parseQueueData(qSnap.data());
            mergAndRender();
          });
        });
        // Limpiar torneos ya no publicados
        Object.keys(pubQueuesLocal).forEach(sid=>{if(!ids.includes(sid)){delete pubQueuesLocal[sid];}});
        mergAndRender();
      });
  ...
  unsubscribeScores = onSnapshot(collection(db,'scores'), async snap=>{
    for(const change of snap.docChanges()){
      ...
      if(d.sid===sessionId){
        ...
        // Score de otro torneo — guardarlo en Firestore para que lo procese quien tenga ese torneo activo
        // Si nadie lo procesa en 60s, borrarlo para no acumular basura
        setTimeout(()=>deleteDoc(change.doc.ref).catch(()=>{}), 60000);
      }
      await dispatchNextFromQueue();
...
async function renderTourneysList(){
  ...
  entries.forEach(([sid,data])=>{
    ...
    row.innerHTML=`<div style="display:flex;align-items:center;gap:0.8rem;flex:1;cursor:pointer" onclick="loadTournament('${sid}')"><div><div style="font-weight:700">${title}</div><div style="font-size:0.75rem;color:var(--text-muted)">${isKO?'Fase Eliminatoria':'Fase de Grupos'}</div></div></div><button class="btn btn-danger" style="padding:0.3rem 0.6rem" onclick="deleteTournament('${sid}',event)">Eliminar</button>`;
...
async function deleteTournament(sid, event){
  ...
  try{
    const pubSnap=await getDoc(doc(db,'config','publishedTorneos'));
    const ids=(pubSnap.data()?.sessionIds||[]).filter(id=>id!==sid);
    await setDoc(doc(db,'config','publishedTorneos'),{sessionIds:ids,updatedAt:serverTimestamp()});
...
function renderDevicesList(){
  ...
  connectedDevices.forEach(dev=>{
    ...
    row.innerHTML=`<div><div class="dev-name">${dev.name}</div><div class="dev-status" style="color:${statusColor}">${statusTxt}</div></div><button class="btn btn-danger" style="padding:0.2rem 0.5rem;font-size:0.75rem" onclick="removeDevice('${dev.clientId}')">✕</button>`;
    list.appendChild(row);
  });
}
...
$('import-file-input').addEventListener('change', async e=>{
  ...
  try{
    ...
    if(ext==='xlsx'||ext==='xls'){
      ...
      const XLSX = await import('https://cdn.jsdelivr.net/npm/xlsx@0.18.5/xlsx.mjs').catch(()=>null);
...
function buildGroupCard(group,gi){
  ...
  hdr.innerHTML=`<span>${group.name}</span><span class="gcard-counter">${doneM}/${allM}</span>`;
  ...
  calcStandings(group).forEach((s,i)=>{
    ...
    tr.innerHTML=`<td><span class="rank">${i+1}</span></td><td>${s.team.name}</td><td>${s.P>0?s.P:'—'}</td><td class="pts-col">${s.P>0?s.Pts:'—'}</td><td><button class="move-btn" onclick="moveTeam(${gi},${i},${i-1})">▲</button><button class="move-btn" onclick="moveTeam(${gi},${i},${i+1})">▼</button></td>`;
  ...
  group.matches.forEach((m,mi)=>{
    ...
    const devTag=live?`<span style="font-size:0.58rem;color:var(--win);margin-left:2px;opacity:0.8">${getMatchDeviceName(gi,mi)||''}</span>`:qPos?`<span style="font-size:0.58rem;color:var(--gold);margin-left:2px;opacity:0.9">#${qPos}</span>`:'';
    item.innerHTML=`${dot}<div class="gm-team">${m.t1.name}</div><div class="gm-score${m.played?'':' pending'}">${s1}–${s2}</div><div class="gm-team r">${m.t2.name}</div>${devTag}${editIcon}`;
...
function buildCard(match, ri, mi, isFinal=false){
  ...
  [match.t1, match.t2].forEach((team, si)=>{
    ...
    if(isBye){
      ...
      slot.innerHTML=`<span class="slot-seed">${team?'#'+team.seed:''}</span><span class="slot-name">${team?team.name:'Por decidir'}</span><span class="slot-win-icon">▲</span>`;
    }
...
function openBracketScorePanel(ri, mi, isGF, isLower, lri){
  ...
  $('bsp-row').innerHTML = `
    <div class="sc-lbl">${match.t1.name}</div>
    <div class="sc-grp">
    ...
    <div class="sc-lbl">${match.t2.name}</div>`;
...
function renderDisplayScreen(data){
  ...
  function buildDvCard(g){
    ...
    const rows=st.map((s,rank)=>{
      ...
      return `<tr${rank===0?' class="dv-row-first"':''}><td><span class="dv-rank${rank===0?' top':''}">${rank+1}</span></td><td class="td-name">${s.team.name}</td><td>${pj}</td><td class="${gdCls}">${gdTxt}</td><td><span class="dv-pts">${pts}</span></td></tr>${sep}`;
    }).join('');
...
window.generarPegatinas = async function(){
  ...
  const bgImg = 'data:image/png;base64,';
...
window.imprimirTicketPartido = function(data){
